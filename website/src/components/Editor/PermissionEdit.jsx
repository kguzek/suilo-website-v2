...
export const PermissionEdit = ({ data, loaded, refetchData, userPerms, allPerms }) => {
  ...
  useEffect(() => {
    ...
    const user = (data.contents ?? []).filter((user) => user.id === currentlyActive).shift();
    if (!user) {
  ...
  const _handleSubmit = (e) => {
    ...
    fetchWithToken(url, 'PUT', {}, { email, canEdit: [...canEdit] }).then((res) => {
